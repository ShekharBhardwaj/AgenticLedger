...
installed, and a bad response changes nothing. This is the only
price-related network call the CLI makes, and only when you run it: the
ledger never fetches anything on its own. Restart the proxy to apply; delete the folder to
